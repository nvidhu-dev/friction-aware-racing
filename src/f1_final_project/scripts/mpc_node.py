...
class MPC(Node):
    ...
    def __init__(self):
        ...
        self.declare_parameters(namespace='', parameters=[
            ('waypoint_file',  'ice_waypoints_mpc.csv'),
    ...
    def load_waypoints(self, filename):
        pkg_path = '/home/nvidia/f1/f1tenth_ws/src/f1_final_project/' #change this to local path to pure_pursuit folder
    ...
    def friction_callback(self, friction_msg):
        if friction_msg.data == 'low':
            print("friction low")
            Q  = np.diag([8.0, 8.0, 3.0, 8.0]) 
            R  = np.diag([0.1, 200.0]) 
            Rd = np.diag([0.1, 200.0]) 
            self.speed_multiplier = 0.7
            self.config.Qk  = Q
            self.config.Qfk = Q   # keep Qf = Q, or expose separately if you want
            self.config.Rk  = R
            self.config.Rdk = Rd
        if friction_msg.data == 'medium':
            print("friction medium")
            Q  = np.diag([13.5, 13.5, 5.5, 13.0])
            ...
            self.config.Rk  = R
            self.config.Rdk = Rd
        if friction_msg.data == 'high':
            print("friction high")
            Q  = np.diag([18, 18, 8, 17.0])
            R  = np.diag([0.5, 400.0])
            Rd = np.diag([0.5, 400.0])
